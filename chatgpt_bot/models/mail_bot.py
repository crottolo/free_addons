...
#################################################################################################
    ...
    def risposta(self, record, body): 
        ...
        odoobot_id = odoobot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        
        mod_response = self.env['mail.channel'].browse(record.id).message_post(body=gpt, message_type='comment', subtype_id=37, author_id=odoobot_id)
