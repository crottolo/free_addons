...
class BancheItaliane(models.Model):
    ...
    _order = "abi, cab, istituto, sportello"
...
#################################################################################################
    ...
    @api.depends('abi', 'cab', 'istituto', 'sportello')
    def _compute_name(self):
        for record in self:
            record.name = f"{record.abi if record.abi else ''} {record.cab if record.cab else ''} {record.istituto if record.istituto else ''} {record.sportello if record.sportello else ''}"
...
#################################################################################################
    ...
    def download_file_github(self):
        ...
        if response.status_code == 200:
            try:
                ...
                if isinstance(data, dict) and 'abicab' in data:
                    ...
                    if isinstance(banche, list):
                        ...
                        self.search([]).unlink()
                        _logger.info(
                            "Tutti i record esistenti sono stati eliminati.")
                        ...
                        _logger.info(
                            "Operazione completata. Totale nuovi record creati: %s", len(banche))
                    else:
                        _logger.warning(
                            "La chiave 'abicab' non contiene una lista come previsto.")
                else:
                    _logger.warning(
                        "La struttura del JSON non è come previsto.")
            ...
            _logger.error(
                "Errore nel download del file. Status code: %s", response.status_code)
            return False
