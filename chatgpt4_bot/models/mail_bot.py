...
class ChatGptBot(models.AbstractModel):
    ...
    def _get_answer(self, record, body, values, command=False):
        ...
        res = super()._get_answer(record, body, values, command=False)

        get_last_message = (
            self.env["mail.channel"].search([("id", "=", record.id)]).message_ids.ids
        )
        msgs = self.env["mail.message"].search(
            [("id", "in", get_last_message)],
            order="id desc",
            limit=20,
        )
        ...
        if body == "#clear":
            self.env["mail.message"].search(
                [("id", "in", get_last_message[1:])],
                order="id desc",
            ).unlink()

            return None  # "cleared last 10 messages"
        ...
        comp = len(contatti.filtered(lambda x: x.is_company))
        ct = len(contatti.filtered(lambda x: not x.is_company))
        if odoobot_state == "chatgpt" and not msg_sys:
            lang = self.env.user.lang
            tz = self.env.user.tz
            local = pytz.timezone(tz)
            now = datetime.datetime.strftime(
                pytz.utc.localize(datetime.datetime.utcnow()).astimezone(local),
                "%Y-%m-%d %H:%M:%S",
            )
            ...
            message_new = [
                {
                    "role": "system",
                    "content": "You are a veterinary assistant and work for this company with name "
                    + self.env.company.name,
                },
                {"role": "system", "content": "Answer same a veterinary assistant"},
                {
                    "role": "system",
                    "content": "qualified in veterinary medicine and surgery",
                },
                # {'role': 'system', 'content': 'Email:'+self.env.user.email},
                {
                    "role": "system",
                    "content": "the date and time now is "
                    + now
                    + " with timezone "
                    + tz,
                },
                ...
                {
                    "role": "system",
                    "content": "At the beginning of the conversation, include the person's name "
                    + self.env.user.name,
                },
                {
                    "role": "system",
                    "content": "Answer Format: html",
                },  # and replace ```json {\} ``` with <pre> and </pre>'},
                {"role": "system", "content": "consulto: https://ciaovet.com/consulto"},
                {
                    "role": "system",
                    "content": "prenotazione: https://ciaovet.com/consulto",
                },
                {
                    "role": "system",
                    "content": "appuntamento: https://ciaovet.com/consulto",
                },
                ...
                {"role": "user", "content": "Mi chiamo " + self.env.user.name},
                {
                    ...
                    "content": "Ciao"
                    + self.env.user.name
                    + " Roberto! Come posso esserti utile?",
                },
                ...
                {
                    "role": "assistant",
                    "content": "puoi fissare un consulto direttamente qui: https://ciaovet.com/consulto",
                },
    ...
    def risposta(self, record, messages):
        openai.api_key = (
            self.env["ir.config_parameter"].sudo().get_param("chatgpt_api_key")
        )
        ...
        for i, part in enumerate(parts):
            if i % 2 == 0:
                ...
                parts[i] = markdown.markdown(
                    parts[i],
                )  # conversione in HTML del testo (ad eccezione del codice markdown)
            else:
