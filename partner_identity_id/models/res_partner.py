...
class ResPartner(models.Model):
    ...
    is_attach_present = fields.Boolean(
        string="Attachment Present",
        compute="_compute_is_attach_present",
    )
    ...
    def _compute_is_attach_present(self):
        for rec in self:
            rec.is_attach_present = rec.identity_ids.filtered(
                lambda x: x.attachment_id and x.is_default,
            )
    ...
    def compute_identity_fields(self):
        for rec in self:
            identity = rec.identity_ids.filtered(lambda x: x.is_default)
            if identity:
                identity = identity[0]
            ...
            if "identity_ids" in vals:
                for command in vals["identity_ids"]:
                    ...
                    if command[0] == 0:  # Create
                        ...
                        if document_type_id:
                            ref_id_card = self.env.ref(
                                "partner_identity_id.identity_card_type",
                            )
                ...
                if command[0] == 0:  # Create
                    ...
                    if document_type_id:
                        ...
                        if document_type_id == ref_id_card.id:
                            ...
                            identity = self.identity_ids.filtered(
                                lambda x: x.document_type_id.id == ref_id_card.id,
                            )
                            if identity:
        ...
        for rec in self:
            if rec.document_issuing_date and rec.document_expiration_date:
                if rec.document_issuing_date > rec.document_expiration_date:
                    raise UserError(
                        _(
                            "The issuing date cannot be greater than the expiration date",
                        ),
                    )
